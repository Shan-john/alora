...
import { Heart, X, MessageCircle, ChevronRight, ShoppingBag } from 'lucide-react';
...
export default function Wishlist() {
  ...
  const generateWhatsAppCheckoutLink = () => {
    ...
    return `https://wa.me/919497711275?text=${encodeURIComponent(text)}`;
  };
        ...
        <div className="max-w-[1200px] mx-auto px-6 lg:px-8 xl:px-12">
          <div className="text-center mb-12">
             ...
             <p className="font-body text-[#666] text-[15px]">
               {totalWishlistItems} {totalWishlistItems === 1 ? 'item' : 'items'} in your list
             </p>
          </div>
          ...
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 bg-white border border-[#e5e5e5]">
              ...
              <h3 className="font-display text-[24px] text-[#222] mb-3">Your wishlist is empty</h3>
              <p className="font-body text-[#666] text-[15px] mb-8">Save your favorite pieces here before deciding.</p>
              <Link to="/shop">
            ...
            <div className="flex flex-col lg:flex-row gap-10 items-start">
              ...
              <div className="w-full lg:w-[65%] bg-white border border-[#e5e5e5]">
                {items.map((item, index) => (
                  ...
                  >
                    <Link to={`/product/${item.id}`} className="shrink-0 w-[120px] h-[120px] bg-[#f8f8f8] flex items-center justify-center p-2 mr-6">
                    ...
                    <div className="flex-1 flex flex-col justify-between py-1">
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <Link to={`/product/${item.id}`} className="font-body text-[16px] font-medium text-[#222] hover:text-[#B8973A] transition-colors line-clamp-2 leading-snug">
                            {item.name}
                          </Link>
                          <p className="font-body text-[14px] text-[#888] mt-2">Qty: 1</p>
                        </div>
                        <button 
              ...
              <div className="w-full lg:w-[35%] bg-white border border-[#e5e5e5] p-8 sticky top-[120px]">
                ...
                <div className="space-y-4 font-body text-[14px] text-[#666] mb-6 border-b border-[#e5e5e5] pb-6">
                  <div className="flex justify-between">
                    <span>Subtotal</span>
                    <span className="text-[#222] font-medium">{formatPrice(totalPrice)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Shipping</span>
